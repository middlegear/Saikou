...
class Aniwatchtv : AnimeApiParser() {

    override val name = "aniwatch"
    override val saveName = "aniwatch"
    ...
    override suspend fun search(query: String): List<ShowResponse> {
        return tryWithSuspend(post = true, snackbar = true) {
            ...
            val res = client.get(
                "$hostUrl/api/aniwatch/anime/search?q=$encoded",
    ...
    override suspend fun loadEpisodes(
        ...
        return tryWithSuspend(post = true, snackbar = true) {
            ...
            val url = "$hostUrl/api/aniwatch/anime/$animeLink"
            val res =
    ...
    override suspend fun loadVideoServers(
        ...
        return tryWithSuspend(post = false, snackbar = true) {
            ...
            val res = client.get(
                "$hostUrl/api/aniwatch/episode/$episodeLink/servers",
                headers = mapOf("x-api-key" to apiKey)
            ...
            fun addServers(version: String, list: List<ServerItem>) {
                list.forEach { item ->
                    ...
                    val embedUrl =
                        "$hostUrl/api/aniwatch/sources/$episodeLink?version=$version&server=${item.serverName}"
